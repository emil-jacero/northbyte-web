...
**🛑 REPO-LOCAL PATCH — this workflow DOES NOT APPLY in this repo. Stop here.**
The `site-change` schema has no specs artifact, there is no `openspec/specs/`, and every
change declares `skip_specs: true` (`catalog_schema.cue` and `catalog.cue` are the spec —
`openspec/config.yaml` Principle I). There is nothing to sync. Do not run these steps and
do not create an `openspec/specs/` directory. This file exists only because `openspec update`
regenerates it from the global workflow profile; report the request and stop.
