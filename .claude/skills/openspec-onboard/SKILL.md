...
(REPO-LOCAL PATCH) A change that declared a `enhancements/` entry in `enhancement.yaml` also logs its landing into that entry's `delivery.yaml` at archive; the `openspec-archive-change` skill's step 3b/3c does it, `openspec archive` alone does not.
```
